...
const DashboardLayout = () => {
    ...
    // Real, separate mobile drawer state - distinct from isCollapsed
    // (the desktop-only icon-vs-expanded toggle). Defaults closed so a
    // mobile visitor's first view shows their actual content, not a
    // drawer covering it.
                ...
                <div style={{ position: 'relative', height: '100vh', width: '100vw', overflow: 'hidden' }}>
                    ...
                    >
                        <Sidebar
                            ...
                            isMobile={isMobile} isMobileNavOpen={isMobileNavOpen} setIsMobileNavOpen={setIsMobileNavOpen}
                        />
                        {/* Real backdrop - only exists at all when the mobile
                            drawer is genuinely open, so it adds zero real
                            DOM/behavior on desktop or when the drawer is
                            closed. Tapping it closes the drawer, matching
                            the standard, expected mobile-drawer convention. */}
                        ...
                        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', height: '100vh', minWidth: 0 }}>
                            <Header setActiveTab={setActiveTab} isMobile={isMobile} onOpenMobileNav={() => setIsMobileNavOpen(true)} />
                            <div style={{ flex: 1, overflowY: 'auto', willChange: 'scroll-position', minWidth: 0 }}>
                                ...
                                <div className="glass-panel" style={{ padding: isMobile ? '16px 12px 24px 12px' : '32px 40px 40px 40px' }}>
                                    {renderActiveModule()}
                                </div>
                            </div>
                        </div>
