...
// clicks.
const SmoothVolumeSlider = ({ value, onChange, disabled }) => {
...
const SmoothRangeSlider = ({ value, min, max, step, onChange, disabled, width = '140px' }) => {
    const [localValue, setLocalValue] = useState(value);
    const rafRef = useRef(null);
    const isDraggingRef = useRef(false);
                ...
                <div style={{ background: 'var(--bg-surface)', border: '1px solid var(--border-premium)', borderRadius: '20px', padding: '24px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
                    ...
                    {isPinConfigured(settings.appPin) && (
                        <div style={{ borderTop: '1px solid var(--border-premium)', paddingTop: '16px' }}>
                            <label style={{ display: 'block', fontSize: '13px', fontWeight: '700', color: 'var(--text-secondary)', marginBottom: '4px' }}>Protected Modules</label>
                            <span style={{ fontSize: '11px', color: 'var(--text-muted)', display: 'block', marginBottom: '10px' }}>
                                Choose which pages require the PIN to open{(settings.lockedModules || []).length === 0 ? ' - none selected yet, so nothing is currently locked' : ''}.
                            </span>
                    ...
                    <LockedApiField label="Spotify Client ID & Secret" pinConfigured={isPinConfigured(settings.appPin)} storedPinHash={settings.appPin} hasValue={!!(settings.spotifyClientId || settings.spotifyClientSecret)}>
                        <div>
                            <label style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '13px', fontWeight: '700', color: 'var(--text-secondary)', marginBottom: '8px' }}>
                ...
                <div style={{
                    ...
                    <div style={{
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexShrink: 0 }}>
                            <SmoothRangeSlider value={settings.glassBlur} min={0} max={40} step={1} onChange={(v) => handleChange('glassBlur', v)} />
                    ...
                    <div style={{
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexShrink: 0 }}>
                            <SmoothRangeSlider value={settings.glassOpacity} min={1} max={15} step={1} onChange={(v) => handleChange('glassOpacity', v)} />
